...
mng_retcode mng_create_StateDataObject (mng_datap      pData,
                                        mng_bool       bConcrete,
                                        mng_bool       bViewable,
                                        mng_uint32     iWidth,
                                        mng_uint32     iHeight,
                                        ...
                                        mng_imagedatap *ppObject)
{
...
#ifdef MNG_OPTIMIZE_OBJCLEANUP
  {
    ...
    mng_retcode iRetcode = create_obj_general (pData, sizeof (mng_imagedata),
                                               (mng_cleanupobject)mng_free_StateDataObject,
                                               MNG_NULL, &pTemp);
    if (iRetcode)
      return iRetcode;
  ...
  pImagedata->sHeader.fCleanup   = (mng_cleanupobject)mng_free_StateDataObject;
...
mng_retcode mng_free_StateDataObject   (mng_datap      pData,
                                        mng_imagedatap pImagedata)
{
#ifdef MNG_SUPPORT_TRACE
...
mng_retcode mng_clone_StateDataObject  (mng_datap      pData,
                                        mng_bool       bConcrete,
                                    ...
                                    mng_imagep *ppObject)
{
  ...
  iRetcode = mng_create_StateDataObject (pData, bConcrete, bViewable,
                                  ...
                                  mng_imagep pImage)
{
  ...
  iRetcode = mng_free_StateDataObject (pData, pImgbuf);
                                       /* drop its own buffer */
  MNG_FREEX (pData, pImage, sizeof (mng_image));
  ...
  else                                 /* create a full clone ! */
  {
    ...
    if (!bAbstract)                    /* determine concreteness from source ? */
      bConcrete = pSource->pImgbuf->bConcrete;
                                       /* create a full clone ! */
    iRetcode = mng_clone_StateDataObject (pData, bConcrete, pSource->pImgbuf, &pImgbuf);
