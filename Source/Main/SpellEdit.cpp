...
LRESULT CSpellEdit::OnTimer(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	...
	if (nIDEvent == m_timer)
	{
		...
		{
			CPen margpen; margpen.CreatePen(PS_SOLID, 0, ::GetSysColor(COLOR_GRAYTEXT));
			hOldPen = dc.SelectPen(margpen);
			POINT pt = {0,0};
			RECT rect;
		...
		CPen mypen; mypen.CreatePen(PS_SOLID, 0, m_ErrColor);
		hOldPen = dc.SelectPen(mypen);
		...
		do
		{
			...
			if (nLineLength > 0)
			{
				...
				do
				{
					...
					CSize size;
					CString word;
					dc.GetTextExtent(line.Left(oldpos), -1, &size);

					CSize wordsize;
					if (pos >= 0)
					{
						word = line.Mid(oldpos, pos-oldpos);
						dc.GetTextExtent(word, -1, &wordsize);
					}
					else
					{
					...
					{    // Undo underlining of previously misspelled word    
						CPen eraserPen; eraserPen.CreatePen(PS_SOLID, 0, ::GetSysColor(COLOR_WINDOW));    
						HPEN hErrorPen = dc.SelectPen(eraserPen);    
						CharPos = PosFromChar(oldpos + nLineIndex);    
						dc.MoveTo(CharPos.x, CharPos.y+wordsize.cy);    
						dc.LineTo(CharPos.x+wordsize.cx, CharPos.y+wordsize.cy);	    
						dc.SelectPen(hErrorPen);
					}				
...
{
	CWindowImpl<CSpellEdit, CEdit>::SubclassWindow(hWnd);
	...
	if (GetModuleFileName(NULL, buf, MAX_PATH))
	{
		...
		do
		{
			GetLocaleInfo(MAKELCID(langId, SORT_DEFAULT), LOCALE_SISO639LANGNAME, buf, countof(buf));
			sFile = buf;
			sFile += _T("_");
			GetLocaleInfo(MAKELCID(langId, SORT_DEFAULT), LOCALE_SISO3166CTRYNAME, buf, countof(buf));
			sFile += buf;
...
LRESULT CSpellEdit::OnContextMenu(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	int nCmd = menu.TrackPopupMenu(TPM_LEFTALIGN | TPM_LEFTBUTTON | 	
		TPM_RETURNCMD | TPM_RIGHTBUTTON, point.x, point.y, m_hWnd);  

	// delete the submenu's
	for (int i=0; i<menuArray.GetSize(); i++)
	{
		delete menuArray[i];
	}
