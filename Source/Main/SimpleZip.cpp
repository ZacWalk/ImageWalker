...
zipFile ZEXPORT zipOpen OF((LPCTSTR pathname, int append));
/*
  Create a zipfile.
...
#ifndef Z_nMaxFILENAMEINZIP
#define Z_nMaxFILENAMEINZIP (256)
#endif
...
local linkedlist_datablock_internal* allocate_new_datablock()
{
    ...
    ldi = (linkedlist_datablock_internal*)IW::Alloc(sizeof(linkedlist_datablock_internal));
    if (ldi!=NULL)
    {
...
local int write_datablock(FILE * fout,
    linkedlist_data* ll)
{
    linkedlist_datablock_internal* ldi;
    ldi = ll->first_block;
...
zipFile ZEXPORT zipOpen (LPCTSTR pathname, int append)
{
    zip_internal ziinit;
    zip_internal* zi;

    ziinit.filezip = 0;
	_tfopen_s(&ziinit.filezip, pathname, (append == 0) ? _T("wb") : _T("ab"));
    ...
    zi = (zip_internal*)IW::Alloc(sizeof(zip_internal));
    if (zi==NULL)
    {
    ...
    
{
    ...
    zi->ci.central_header = (char*)IW::Alloc((uInt)zi->ci.size_centralheader);
...
int ZEXPORT zipWriteInFileInZip (zipFile file, const voidp buf, unsigned len)
{
    zip_internal* zi;
    int err=ZIP_OK;
...
bool IW::CSimpleZip::Create(const CString &strZipFileName)
{
	USES_CONVERSION;

	assert(m_pZipFile == 0);
	m_pZipFile = zipOpen(strZipFileName, 0);
	return m_pZipFile != 0;
}

bool IW::CSimpleZip::Close()
{
	if (m_pZipFile)
	{
		int errclose = zipClose((zipFile)m_pZipFile,NULL);
	}
...
bool IW::CSimpleZip::AddFile(IW::IStreamIn *pStreamIn, const CString &strFileNameIn, IW::FileTime ft, IW::IStatus *pStatus)
{
	USES_CONVERSION;

	zip_fileinfo zi;
	IW::MemZero(&zi, sizeof(zi));

    ft.FileTimeToDosDateTime(((LPWORD)&zi.dosDate)+1,((LPWORD)&zi.dosDate)+0);

	CString strFileName;
	for(int i = 0; i < m_arrayFolderNames.GetSize(); i++)
	{
		strFileName += m_arrayFolderNames[i];
		strFileName += _T("/");
	}

	strFileName += strFileNameIn;

    int err = zipOpenNewFileInZip((zipFile)m_pZipFile, CT2A(strFileName), &zi,NULL,0,NULL,0,NULL, Z_DEFLATED, Z_BEST_COMPRESSION);
    if (err != ZIP_OK)
    {
        ATLTRACE(_T("error in opening %s in zipfile\n"), strFileName);
        return false;
    }
	...
	DWORD dwRead = 0;
	BYTE buffer[nBufferSize + 1];

	int nWriteByteCount = 0;

	// Read the first buffer full
	pStreamIn->Read(buffer, nBufferSize, &dwRead);

	while(dwRead > 0)
	{
		//TODO: Process the "Only upload newer files" option

		// Write
		err = zipWriteInFileInZip ((zipFile)m_pZipFile, buffer, dwRead);
        if (err<0)
        {
            //We could not write the file in the ZIP-File for whatever reason.
            ATLTRACE(_T("Could not write %s to zip file\n"), strFileName);
            return false;
        }

		nWriteByteCount += dwRead;	
	...
	err = zipCloseFileInZip((zipFile)m_pZipFile);
	if (err != ZIP_OK)
    {
        ATLTRACE(_T("failed to close\n"), strFileName);
        return false;
    }
...
bool IW::CSimpleZip::AddFile(const CString &strFileIn, const CString &strFileName, IW::FileTime ft, IW::IStatus *pStatus)
{
	IW::CFile f;
	if (f.OpenForRead(strFileIn))
	{
		return AddFile(&f, strFileName, ft, pStatus);
	}

	return false;
}

bool IW::CSimpleZip::AddFile(const CString &strFileIn)
{
	IW::FileTime ft = IW::FileTime::FromFile(strFileIn);
	return AddFile(strFileIn, IW::Path::FindFileName(strFileIn), ft, IW::CNullStatus::Instance);
}

///////////////////////////////////////////////////////////////////////
// The following methods are events called by ImageWalker during
// processing items in the folder. Together thay make up
// a IItemStream interface.
//

bool IW::CSimpleZip::StartFolder(IW::Folder *pFolder, IW::IStatus *pStatus)
{
...
bool IW::CSimpleZip::StartItem(IW::FolderItem *pItem, IW::IStatus *pStatus)
{
	IW::FolderStreamIn streamIn(pItem);	

	if (!AddFile(&streamIn, pItem->GetFileName(), pItem->GetLastWriteTime(), pStatus))
	{
		return false;
	}

	// Set the status
	pStatus->SetMessage(App.LoadString(IDS_ZIPPED));

	return true;
}

bool IW::CSimpleZip::EndItem()
{
	return true;
}

bool IW::CSimpleZip::EndFolder()
{
	return true;
}
...
bool IW::CSimpleZip::QueryCancel()
{
	return false;
}
