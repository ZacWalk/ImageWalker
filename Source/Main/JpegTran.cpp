...
* transupp.c
*
* Copyright (C) 1997, Thomas render. Lane.
...
#include "JpegTran.h"		/* My own external interface */
#include "LoadJpg.h"
#include "Exif.h"
...
static int GountSteps(j_compress_ptr dstinfo)
{
	int nSteps = 0;
		 ...
		 /* Crop.  This is only used when no rotate/flip is requested with the crop. */
{
	...
	int nStep = 0, nTotalSteps = GountSteps(dstinfo);
	...
	*/
		...
		GETJOCTET(srcinfo->marker_list->data[5]) == 0) 
	{
		...
		if (dstinfo->image_width != srcinfo->image_width ||
			...
			Exif::FixExif(srcinfo->marker_list->data + 6,
			srcinfo->marker_list->data_length - 6,
...
static char *exif_signature = "Exif";

static const unsigned xmp_signature_length = strlen(xmp_signature) + 1;
static const unsigned iptc_signature_length = 24;
static const unsigned exif_signature_length = 4;
...
bool IsIptcBlob(jpeg_saved_marker_ptr marker)
{
	...
	return memcmp(iptc_signature, marker->data, 12) == 0;
}
...
{
	return marker->marker == ICC_MARKER && marker->data_length > 0;
}
...
IW::MetaData LoadIptcBlob(jpeg_saved_marker_ptr marker)
{
	...
	while ((data[nStart] != 0x1c || data[nStart+1] != 0x02) && nStart < nLength)
	{
		nStart += 1;
	}			

	return IW::MetaData(IW::MetaDataTypes::PROFILE_IPTC, data + nStart, nLength - nStart);
}
...
{
	return IW::MetaData(IW::MetaDataTypes::PROFILE_ICC, marker->data, marker->data_length);
}
...
void WriteIptcBlob(j_compress_ptr dstinfo, const IW::MetaData &blob)
{
	...
	if (size > 0)
	{
		...
		for (unsigned i=0; i < size; i+=65500u)
		{
			...
			unsigned nWriteSize = nLength+nRoundup+nTagLength; // round up for Photoshop

			IW::CAutoFree<BYTE> pProfile((LPBYTE)IW::Alloc(nWriteSize));

			IW::MemCopy(pProfile, iptc_signature, iptc_signature_length);
			pProfile[13]=0x00;
			pProfile[24]=nLength >> 8;
...
void WriteIccBlob(j_compress_ptr dstinfo, const IW::MetaData &blob)
{
	...
	if (size > 0)
	{
		jpeg_write_marker(dstinfo, ICC_MARKER, blob.GetData(), size);
	}
}
...
void WriteExifBlob(j_compress_ptr dstinfo, const IW::MetaData &blob)
{
	...
	{
		LPCBYTE p = blob.GetData();
		IW::CAutoFree<BYTE> pProfile((LPBYTE)IW::Alloc(size));
