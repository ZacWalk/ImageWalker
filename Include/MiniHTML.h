...
public:
			...
			case CMD_DRAW:
				{
					...
					if( m_bWordWrap ) 
					{
						...
						{
							int nLen = action.cchMax - nCopied;
							dc.GetTextExtent(pstr + nCopied, nLen, &sizeText);
							while( (nLen > 0) && (rcPos.left + sizeText.cx > rc.right))
							{
								while( nLen > 0 ) 
								{
									nLen--;
									if( nLen == 0 || *(pstr + nCopied + nLen - 1) == _T(' ') ) break;
									dc.GetTextExtent(pstr + nCopied, nLen - 1, &sizeText);
								}
							}	
							...
							{
								dc.DrawText(pstr + nCopied, nLen, &rcPos, DT_SINGLELINE | DT_TOP | DT_NOPREFIX | uStyle);
								nCopied += nLen;

								if( nLen < action.cchMax - nCopied ) 
								{
									rcPos.left = rc.left;
									rcPos.top += action.iHeight;
								}
		...
		HFONT hFont)
	{
		...
		f.lf.lfQuality = ANTIALIASED_QUALITY;
...
public:
	...
	LRESULT OnCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		SetWindowText(_T("The Road to <b>Enlightenment</b>.<br>Littering a dark and dreary road lay the past <i>relics</i> of browser-specific tags, incompatible DOMs, and broken CSS support."));
		SetToolTipText(_T("Its time to go home"));
