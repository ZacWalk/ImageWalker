...
public:
   ...
   typedef struct tagPANEL
   {
      ...
      int Image;                    // Image Index into ImageList
   ...
   int InsertItem(HWND hWndView, LPCTSTR pstrTitle, int Image = -1, COLORREF clrText = CLR_INVALID)
   {
      ATLASSERT(::IsWindow(hWndView));
      ATLASSERT(!::IsBadStringPtr(pstrTitle,-1));
      ATLASSERT(Image==-1 || !m_Images.IsNull()); // Need to assign ImageList first
   ...
   RECT GetPanelRect(int iIndex, BOOL bIncludeButton) const
   {
      ...
      for( int i = 0; i <= iIndex; i++ ) {
         if( m_aPanels[i].Image >= 0 ) rc.top += m_cyButton - m_cyBar;
         rc.top += m_cyBar;
         if( i == iIndex ) {
      ...
      else
      {
         ...
         if( dcMem.GetDeviceCaps(BITSPIXEL) > 8 ) {
            ...
            for( int i = 0; i < nSteps; i++ ) {
               ...
               BYTE bR = (BYTE) ((IW::GetR(clrBar1) * (nSteps - i) +
                          IW::GetR(clrBar2) * i) >> nShift);
               BYTE bG = (BYTE) ((IW::GetG(clrBar1) * (nSteps - i) +
                          IW::GetG(clrBar2) * i) >> nShift);
               BYTE bB = (BYTE) ((IW::GetB(clrBar1) * (nSteps - i) +
                          IW::GetB(clrBar2) * i) >> nShift);
   ...
   void DoPaint(CDCHandle dc, RECT& rect)
   {
      ...
      for( int i = 0; i < m_aPanels.GetSize(); i++ ) {
         ...
         // Add icon
         if( panel.Image >= 0 && !m_Images.IsNull() ) {
            SIZE szIcon;
            m_Images.GetIconSize(szIcon);
            POINT pt = { rcItem.left + 2, rcItem.top + 2 };
            if( !panel.bEnabled && !m_ImagesGrey.IsNull() ) {
               m_ImagesGrey.Draw(dc, panel.Image, pt, ILD_TRANSPARENT);
            }
            else {
               m_Images.Draw(dc, panel.Image, pt, ILD_TRANSPARENT);
            }
